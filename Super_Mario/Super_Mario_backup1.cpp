	...
	if(randomNum%odds==0)
	{
		...
		for (int k=0; k<5; k++)
		{
			if(randomNum%5==k)
			{
				...
				for(int i=0; i<5; i++)
				{
					if(Box[0][0][i]<=-1 && tempCount==5 && gpipe1[0][1]<VDIM && gpipe2[0][1]<VDIM && gpipe3[0][1]<VDIM)
					{
						Box[0][0][i]=VDIM;
						Box[1][0][i]=VDIM;
						Box[0][1][i]=VDIM+1;
...
void nextColumn()
{
	...
	else if (randomNum%7==0)
	{
		...
		if (gpipe2[0][0]<=-1 && gpipe1[0][1]!=VDIM && gpipe3[0][1]!=VDIM && randomNum%3==1 && tempCount==5)
		{
			gpipe2[0][0]=VDIM;
			gpipe2[1][0]=VDIM;
			gpipe2[2][0]=VDIM;
		...
		if (gpipe3[0][0]<=-1 && gpipe2[0][1]!=VDIM && gpipe1[0][1]!=VDIM && randomNum%3==2 && tempCount==5)
		{
			gpipe3[0][0]=VDIM;
			gpipe3[1][0]=VDIM;
			gpipe3[2][0]=VDIM;
...
{
	refresh();
	int secretCoinCount=rand()%20+1;
	char secretMovement=' ';
	bool secretGameOver=false;

	for (int i=0; i<5; i++)
	{
		if (coin[i][2]<=0 && secretCoinCount>0)
		{
	...
	while(secretGameOver==false)
	{
		...
		printw("\n\n score: %i\t lives: %i\t coinCount: %i\t speed: %i\t secretCoinCount: %i\n", score, lives, coinCount, speed, secretCoinCount);
		printw("vPlayer: %i\n",vPlayer);
	}
...
//	cin.ignore(1);
	...
	while(gameOver==false)
	{
		...
		if (count%(5-score/500)==0 || vPlayer>=VDIM/2)
		{
			shiftLeft();
			nextColumn();
		}
...
//		}
		...
		//100 coins=1life
		if (coinCount%100==0 && coinCount!=0)
		{
			lives++;
		}
...
//		}
		...
		printw("\n\n score: %i\t lives: %i\t coinCount: %i\t speed: %i\t", score, lives, coinCount, speed);
		printw("vPlayer: %i\n",vPlayer);
